...
## 3. Supabase Edge Function (AI Proxy)

The `ai-proxy-chat-rag` edge function is the central brain of this project, handling both LLM generation and vector search.
...
3.  **Set Environment Secrets**:
    The edge function requires several environment variables to communicate with Gemini and the Supabase DB.
    ```bash
...
4.  **Deploy the Function**:
    ```bash
    npx supabase functions deploy ai-proxy-chat-rag --no-verify-jwt
    ```
    *Note: The `--no-verify-jwt` flag is used because the function implements its own internal authentication check using explicitly verified user tokens via `supabase.auth.getUser()`.*

## 4. Helper SQL Files In This Repo
...
| `ai-proxy-chat-rag` | The unified API for RAG, Grounding, and AI responses |
